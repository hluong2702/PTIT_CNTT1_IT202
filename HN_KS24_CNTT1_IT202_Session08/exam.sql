...
SELECT room_type, COUNT(room_id) 'Số lượng phòng'
FROM rooms
GROUP BY room_type;
...
SELECT g.guest_name, COUNT(b.booking_id) 'Số lần đặt'
FROM bookings b
JOIN guests g ON g.guest_id = b.guest_id
GROUP BY b.guest_id;

-- Tính doanh thu của mỗi phòng, với công thức: “Doanh thu = số ngày ở× giá thuê theo ngày”
SELECT r.room_id, SUM(DATEDIFF(b.check_out,b.check_in) * r.price_per_day)
FROM bookings b
JOIN rooms r ON r.room_id = b.room_id
GROUP BY b.room_id;

-- Hiển thị tổng doanh thu của từng loại phòng
SELECT r.room_type, SUM(DATEDIFF(b.check_out,b.check_in) * r.price_per_day) 'Doanh thu'
FROM bookings b
JOIN rooms r ON r.room_id = b.room_id
...
SELECT r.room_type, COUNT(b.room_id) 'Số lần đặt'
FROM bookings b
JOIN rooms r ON r.room_id = b.room_id
GROUP BY r.room_type
ORDER BY COUNT(b.room_id) DESC LIMIT 1;
